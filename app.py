...
def handle_postback(event):
    ...
    # 查詢關注股票清單
        ...
        user_id = event.source.user_id
        response = get_watchlist(user_id)  
        reply_messages.append(TextMessage(text=response)) # 不繼續觸發quickReply
    ...
    # 走勢圖查詢
        ...
        stock_name = parts[2] if len(parts) > 2 else stock_code  # 確保 `stock_name` 存在
